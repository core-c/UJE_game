...
type
  ...
  TObjTexture = class(TObject)
                private
                  ...
                  procedure AddLoadedTexture(Filename: string; TextureHandle: GLuint);
...
var F : File;
  ...
  begin
    case PixelSize of
      ...
      4 : begin //RGBA of ABGR
            if SwapColors then begin
              PixelData[i].R := pixels[(i*PixelSize+3) mod Modulo];  //A
              PixelData[i].G := pixels[(i*PixelSize+2) mod Modulo];  //B
              PixelData[i].B := pixels[(i*PixelSize+1) mod Modulo];  //G
              PixelData[i].A := pixels[(i*PixelSize) mod Modulo];    //R
            end else begin
              PixelData[i].R := pixels[(i*PixelSize) mod Modulo];    //R
              PixelData[i].G := pixels[(i*PixelSize+1) mod Modulo];  //G
              PixelData[i].B := pixels[(i*PixelSize+2) mod Modulo];  //B
...
begin
  ...
  if not TextureAlreadyLoaded(FN, Result) then begin
    ...
    if Result>0 then AddLoadedTexture(FN, Result);
  end;
end;
...
procedure TObjTexture.AddLoadedTexture(Filename: string; TextureHandle: GLuint);
var L: integer;
begin
